"""
Transparent risk scoring for the rule-status (headline) anomaly view.

    Risk(α) = σ(c(α)) · Φ(α),   Φ(α) = Σ_k β_k · φ̂_k(α),   k ∈ {ρ, ε, π}

This module scores the *filtered* findings produced by the rule-status
pipeline (``core/anomaly.py --rule-status``), i.e. the findings that
survive the two post-processing conventions of the paper:

    1. reporting convention — generalization rows sharing a near-universal
       allow anchor are collapsed into ONE structural finding per anchor;
    2. negation paradox      — IPImen conflicts driven by negated fields are
       excluded.

The input is ``results/anomalies_rule_status.jsonl`` (same JSON schema as
``results/anomalies.jsonl``).  The output mirrors ``risk.py``:

    results/scored_anomalies_rule_status.jsonl
    results/scoring_summary_rule_status.json

No machine learning is involved.  The defaults are deliberately neutral
(β_ρ = β_ε = β_π = 1/3).
"""
...
class _FindingView:
    """Minimal read-only view over a finding (either a Finding dataclass or
    a plain dict loaded from a JSONL file), exposing the attributes that the
    scorer needs."""

    __slots__ = ("vendor", "anomaly_type", "rules", "details", "explanation")

    def __init__(self, finding):
        if isinstance(finding, dict):
            self.vendor = finding.get("vendor", "")
            self.anomaly_type = finding.get("anomaly_type", "")
            self.rules = finding.get("rules", [])
            self.details = finding.get("details", {})
            self.explanation = finding.get("explanation", "")
        else:
            self.vendor = finding.vendor
            self.anomaly_type = finding.anomaly_type
            self.rules = finding.rules
            self.details = finding.details
            self.explanation = finding.explanation


# ── severity σ (works on both Finding dataclass and _FindingView) ────────────
...
def score_policy(
    ...
    Findings may be ``Finding`` dataclass instances (as produced by
    ``core.anomaly.detect_all``) or plain dicts loaded from
    ``anomalies_rule_status.jsonl``.
    """
    ...
    for f in findings:
        view = _FindingView(f)
        involved = _finding_rules(view, by_seq)
        raw.append((
            view,
            blast_log(involved),
...
def _load_findings(path: Path) -> list[dict]:
    """Load findings from a JSONL file (either anomalies.jsonl or
    anomalies_rule_status.jsonl)."""
    findings: list[dict] = []
    if not path.exists():
        return findings
...
def run(out_dir: Optional[Path] = None) -> dict:
    """Score the rule-status (filtered) findings and persist the ranked output.

    Reads ``results/anomalies_rule_status.jsonl`` (produced by
    ``core/anomaly.py --rule-status``) and writes:

        results/scored_anomalies_rule_status.jsonl
        results/scoring_summary_rule_status.json
    """
    out_dir = out_dir or _default_results_dir()
    out_dir.mkdir(parents=True, exist_ok=True)

    findings_path = out_dir / "anomalies_rule_status.jsonl"
    if not findings_path.exists():
        raise FileNotFoundError(
            f"{findings_path} not found. Run `python code/core/anomaly.py "
            f"--rule-status` first to produce the filtered findings."
        )
    ...
    for vendor in VENDORS:
        csv_path = out_dir / "rule_status" / f"{vendor}_rule_status.csv"
        if not csv_path.exists():
            continue
        rules = load_policy(csv_path, enabled_only=True)
        vendor_findings = [
            f for f in _load_findings(findings_path)
            if f.get("vendor") == vendor
        ]
        scored = score_policy(vendor_findings, rules)
    ...
    with open(out_dir / "scored_anomalies_rule_status.jsonl", "w", encoding="utf-8") as fh:
        for s in all_scored:
            fh.write(json.dumps(s, ensure_ascii=False) + "\n")
    with open(out_dir / "scoring_summary_rule_status.json", "w", encoding="utf-8") as fh:
